...
if __name__ == "__main__":
    ...
    rospy.init_node('tf_listener_target', log_level=rospy.INFO)

    listener = tf.TransformListener()

    # create a publisher to publish the transform as a message
    publisher = rospy.Publisher('/transform_target', TransformStamped, queue_size=10)

    while not rospy.is_shutdown():
        try:
            (trans, rot) = listener.lookupTransform('excavator/base_link', 'excavator/link_5', rospy.Time(0))
            # create a TransformStamped message
            transform_msg = TransformStamped()
            transform_msg.header.stamp = rospy.Time.now()
            transform_msg.header.frame_id = 'excavator/base_link'
            transform_msg.child_frame_id = 'excavator/link_5'
